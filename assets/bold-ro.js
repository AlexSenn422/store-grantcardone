...
BOLD.helpers.moneyFormatter = BOLD.helpers.moneyFormatter || {
	formatMoneyFromTemplate: function formatMoneyFromTemplate(format, price, quote) {
		...
		if (price === "") {
			...
			//format to be $12.00
			if (format.indexOf("") != -1) {
				money = money.replace("", _this.formatMoney(price, 2, '.', ','));
			}
			//format with decimal so $12,00
			if (format.indexOf("") != -1) {
				money = money.replace("", _this.formatMoney(price, 2, ',', ','));
			...
			//format no decimal with space separator so 1 342
			if (format.indexOf("") != -1) {
				money = money.replace("", _this.formatMoney(price, 0, '', ' '));
			}
			//format no decimal with space separator so 1,342
			if (format.indexOf("") != -1) {
				money = money.replace("", _this.formatMoney(price, 0, '', ' '));
...
BOLD.recurring_orders.setupEventListeners = function(){
  if(BOLD.common && BOLD.common.eventEmitter){
    ...
    if(window.jQuery && typeof jQuery().on === 'function' && !BOLD.recurring_orders.jQuerySetup){
      ...
      jQuery(document).off('submit', 'form[action*="/checkout"]:not([action*="/tools/checkout"])', BOLD.helpers.triggerCheckoutEvent);
      ...
      jQuery(document).on('submit', 'form[action*="/checkout"]:not([action*="/tools/checkout"])', BOLD.helpers.triggerCheckoutEvent);
...
BOLD.recurring_orders.updateRecurringWidget = function(data){
  ...
  if(!custom_button){
    ...
    if(submit_button.previousElementSibling && submit_button.previousElementSibling.tagName != 'INPUT' && (!submit_button.previousElementSibling.className || submit_button.previousElementSibling.className.indexOf('product_rp_div') === -1 )){
